...
module.exports = function (config, contentFormatter, contentHandler) {
    ...
    function processJson(data, callback){
        ...
        doctorFormatter.formatDoctorFiles(contentItem, null, 'siteBuild', function (err, item) {
          amphora.handleAmphoraContent(contentItem, null, 'siteBuild', function () {
            ...
            .on('complete', function(data, response) {
                if (data instanceof Error || response.statusCode !== 200) {
                    ...
                    try {
                        ...
                        async.eachLimit(ids, 20, fetchItem, callback );
                    } catch (error) {
                        callback(error);
                    }
                }
